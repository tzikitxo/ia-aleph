...
(function(){
	...
	storage.configure('game.gameModeStatus',{
		compress:false,
		encode:false
	});
	...
	function resizeImage(url, maxWidth, maxHeight, callback) {
		...
		sourceImage.onload = function() {
			try{
				var sourceWidth=Number(sourceImage.width),sourceHeight=Number(sourceImage.height),newWidth=sourceWidth,newHeight=sourceHeight;
				if(newWidth>maxWidth){
					newWidth=maxWidth;
					newHeight=Math.round(sourceHeight*newWidth/sourceWidth);
				}
				if(newHeight>maxHeight){
					newHeight=maxHeight;
					newWidth=Math.round(sourceWidth*newHeight/sourceHeight);
				}
				log('resizing image from ',sourceWidth,'x',sourceHeight,' to ',newWidth,'x',newHeight);
	...
	function setGamePhoto(gamePhoto,skipResize){
		var postResize=function(newGamePhoto){
			currentGame.gamePhoto=newGamePhoto;
			clearPhotoIcons();
			buildGameControlScreen();             
		};
		if(skipResize){
			postResize(gamePhoto);
		}else{
			resizeImage(gamePhoto, 750, 750, postResize);
		}
	}
	if(device.hasCordova()){
		$('#gamePhotoCameraButton').bind('touchstart',function(){
			...
			navigator.camera.getPicture(function(imageData){
				log('received image from camera ( size : ',imageData?imageData.length:undefined,' )');
				log('received image from camera : ',imageData);
				setGamePhoto( "data:image/jpeg;base64," + imageData,true);
			},function(e){
				log('camera error ',e)
			},{
				targetWidth:750,

...
(function(){
	...
	armyList.buildHtmlPrintList=function(){
		...
		$.each([{
			...
			onclick:'$(\'body\').toggleClass(\'hideListInfo\')',
			defaultOn:true
		},{
		...
		function buildCostText(cost,swc,models){
			return cost+' points | '+swc+' swc ( '+models+' models )';
		}
		...
		function buildListRecord(model,skipGobal){
			...
			if(specs["CH: Limited Camouflage"] || specs["CH: TO Camouflage"] || specs["CH: Camouflage"]
				...
				|| specs["AD: Parachutist"] || specs["AD: Combat Jump"]  || specs["AD: Airborne Infiltration"] || specs["AD: Tactical Jump"]){
				modelContainer.addClass('privateInfo');
				hiddenCost+=Number(model.get('cost'));
				hiddenSwc+=Number(model.get('swc'));
				hiddenModels++;
		...
		$.each(modelRecords,function(index,listRecord){
			buildListRecord(listRecord.model);
		});
		
		hiddenListInfo.text(buildCostText(armyList.pointCount-hiddenCost,armyList.swcCount-hiddenSwc,armyList.modelCount-hiddenModels));
